...
function ConsultationPage() {
  ...
  // Auth resolves asynchronously AFTER mount ("Loading your account…" first),
  // so the one-time useState initialiser above can miss it. Re-sync the
  // account fields once the user object arrives — but only fill EMPTY fields
  // so we never clobber anything the visitor already typed.
  useEffect(() => {
    if (!user) return;
    const m = (user.user_metadata ?? {}) as { full_name?: string; name?: string };
    setForm((f) => ({
      ...f,
      name: f.name || m.full_name || m.name || "",
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Source of truth = the live DOM values (browser/password-manager autofill
    // can skip React onChange, which would make state look "empty"), MERGED
    // with React state so neither path can report a filled field as empty.
    const fd = new FormData(e.currentTarget);
    const read = (key: keyof Requirements) => {
      const fromDom = fd.get(key);
      const dom = typeof fromDom === "string" ? fromDom.trim() : "";
      const state = (form[key] ?? "").toString().trim();
      return dom || state;
    };
    ...
    try {
      ...
      // The database insert succeeded — always show the normal success message
      // even if the notification email later failed (the server logs that).
